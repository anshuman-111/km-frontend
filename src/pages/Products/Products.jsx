...
const Products = ({ phone }) => {
    ...
    const handleCategoryClick = (title) => {
        nav(`/products/${title.replace(/\s+/g, "-")}`);
        setSelection(title);
        setSearchList([]);
        setSearchInput("");
        setShowNav("");
    ...
    useEffect(() => {
        if (!loading && categorySelection !== undefined) {
            data?.forEach((cat) => {
                if (
                    cat?.attributes?.title?.toLowerCase() === categorySelection
                ) {
    ...
    useEffect(() => {
        if (categoryTitle !== undefined) {
            setSelection(categoryTitle?.replace(/-/g, " "));
            console.log(categoryTitle?.replace(/-/g, " "));
        } else {
            if (data) {
                setSelection(data[0]?.attributes?.title.toLowerCase());
            } else {
                setSelection("foodie cakes");
            }
        }
    }, [categoryTitle, data]);
